...
void create_inet_socket(char *ip, int port) {
...
void create_unix_socket(char *unix_path) {
    ...
    sprintf(bind_addr.sun_path, "client_%d", getpid());
    path = calloc(20, sizeof(char));
    sprintf(path, "client_%d", getpid());
...
void setup_connection(char *argv[]) {
    ...
    if (strcmp(argv[2], "unix") == 0) {
        ...
        create_inet_socket(argv[3], port);
    }
    else {
